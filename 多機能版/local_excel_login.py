...
k = 1
while True:
    if os.path.isfile('Excel//ad' + str(k) + '.xlsx'):
        k = k + 1
    else:
        file_name = 'Excel//ad' + str(k) + '.xlsx'
...
#options.add_argument('--user-data-dir=C:\\Users\\tisk0\\PycharmProjects\\youtube_ad\\User Data')
...
while True:
    ...
    try:
        ...
        if ct_error > 5:
            ...
            #右クリック
            #pyautogui.rightClick(650, 516)
            video_element = driver.find_element_by_css_selector('.ytp-player-content.ytp-iv-player-content')
            webdriver.ActionChains(driver).context_click(video_element).perform()
            # 786 838
            #pyautogui.click(786, 838)
            syosaitouke_elements = driver.find_elements_by_css_selector('.ytp-menuitem-label')
